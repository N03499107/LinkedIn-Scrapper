...
driver = webdriver.Firefox(executable_path = r"C:\Users\bin\geckodriver-v0.11.1-win64\geckodriver.exe")
...
#csv file
outfile = io.open('linkedin_output_certificate.csv','ab')
...
with open('profiles_sorted') as f:
	...
	######CERTIFICATION
        try:
            certificate = driver.find_elements_by_xpath('//div[@id="profile-certifications"]/div[2]/ul/li/div')
            for c in certificate:
                cname=c.find_element_by_tag_name('h4').text
                output.append(cname.encode('ascii','ignore'))
        except NoSuchElementException:
