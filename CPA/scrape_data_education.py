...
#csv file
outfile = io.open('linkedin_output_education.csv','ab')
...
with open('profiles_sorted') as f:
    ...
    for line in f:   
        ...
        ######EDUCATION
        try:
            education = driver.find_elements_by_xpath('//div[@id="profile-education"]/div[2]/ul/li/div')
            for school in education:
                school_name = school.find_element_by_tag_name('h4').text
                at = school.find_element_by_tag_name('h5').text
                yr = school.find_element_by_tag_name('p').text
                output.append(school_name.encode('ascii','ignore'))
